...
class(bob_dylan_words_filtered)
dim(bob_dylan_words_filtered)
...
full_word_count %>%
  ...
  left_join(bob_dylan_orig, by = "song") %>%
  select(Song = song, 
...
wordcloud2(bob_dylan_words_counts[1:300, ], figPath = "guitar_icon.png", 
           color = "random-dark", size = 1.5)

letterCloud(bob_dylan_words_counts[1:300, ], word = "BOB DYLAN", size = 2)
